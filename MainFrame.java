package bsu.rfe.java.group8.lab2.DubatovkaM.var3a;
...
public class MainFrame extends JFrame {
    ...
    public Double calculate1(Double x, Double y, Double z) {
        return Math.pow(Math.cos(Math.exp(x))+Math.log(Math.pow(1+y,2))+ Math.sqrt(Math.exp(Math.cos(x))+Math.pow(Math.sin(Math.PI*z),2))+Math.sqrt(1/x)+Math.cos(y*y),Math.sin(z));
    }
...
//labelResult = new JLabel("0");
        ...
        MP.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent ev) {
                ...
                textFieldSum.setText(String.format("%.5f", sum).toString());
            }
        });
